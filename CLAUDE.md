...
The app has four API routes:
- `POST /api/claude` — proxies to Anthropic's API (Claude Haiku) for the AI assistant; uses forced tool use (`respond_to_customer`) to return structured `{ text, productIds[] }`
- `POST /api/groq` — fallback assistant route proxying to Groq (llama-3.3-70b-versatile)
- `POST /api/notify` — sends transactional emails via Resend (`order_placed`, `order_ready`, `password_reset`, `password_changed`)
...
**Staff session** — `src/lib/staffAuth.ts` manages a separate staff session in localStorage under `"simba-staff-session"` (independent of the main Zustand store). The `/staff/login` page uses the `useStaffUser()` hook from this module rather than the shared auth store.
...
ANTHROPIC_API_KEY      # Primary: Claude Haiku via Anthropic SDK
